...
try {
    ...
    Write-Host "[*] Waiting for role propagation (10s)..." -ForegroundColor Gray
    ...
    if (Test-Path $lambdaDir) {
        ...
        # Environment variables via JSON file
        $lambdaEnv = @{
            Variables = @{
        ...
        $lambdaEnv | Out-File -FilePath "$env:TEMP\lambda-env.json" -Encoding ASCII
        ...
        aws lambda create-function `
          ...
          --timeout 30 `
          --environment "file://$($env:TEMP)\lambda-env.json" `
          --region $AWS_REGION

        Pop-Location
        Write-Host "[+] Lambda function deployed." -ForegroundColor Green
    } else {
        Write-Warning "Lambda directory not found at $lambdaDir. Skipping Lambda deployment."
    }
    ...
    if (!(Test-Path $certDir)) { New-Item -ItemType Directory -Path $certDir -Force }

    $iotKeysJson = (aws iot create-keys-and-certificate `
      --set-as-active `
    ...
    $iotKeys = $iotKeysJson | ConvertFrom-Json
    ...
    $funcArn = (aws lambda get-function --function-name WaterLeakProcessor --region $AWS_REGION --query Configuration.FunctionArn --output text)
    ...
    Write-Host "[!] AWS Setup failed: $($_.Exception.Message)" -ForegroundColor Red
}
